...
public class RaySample : MonoBehaviour {
    ...
    {
        string dir = Application.dataPath + "Art/Texture/Export/" + roleName;
    ...
    // === Added by Assistant: Atlas export helpers ===
    public Color[] CaptureFrameColors()
    {
        var t = StartSample();
        return CreateFrame(t);
    }

    public string ExportAtlas(string roleName, string name, System.Collections.Generic.List<Color[]> frames, int columns, bool fixImporter = true, bool startFromTop = false)
    {
        if (frames == null || frames.Count == 0)
        {
            Debug.LogWarning("ExportAtlas: no frames to export.");
            //return;
        }
        int frameW = SampleWidth;
        int frameH = SampleHeight;
        if (columns <= 0) columns = frames.Count;
        int rows = Mathf.CeilToInt(frames.Count / (float)columns);
        Texture2D atlas = new Texture2D(columns * frameW, rows * frameH, TextureFormat.ARGB32, false);
        ...
        for (int i = 0; i < frames.Count; i++)
        {
            ...
            var colors = frames[i];
            if (colors == null || colors.Length != frameW * frameH) continue;
            atlas.SetPixels(x, y, frameW, frameH, colors);
        }
        atlas.Apply();
        string dir = Application.dataPath + "/Export/" + roleName;
        System.IO.Directory.CreateDirectory(dir);
        byte[] bytes = atlas.EncodeToPNG();
        string fileName = name + "_atlas.png";
        string path = System.IO.Path.Combine(dir, fileName);
        System.IO.File.WriteAllBytes(path, bytes);
        string assetRelPath = "Assets/Export/" + roleName + "/" + fileName;
        Debug.Log("Atlas saved to: " + path);
        #if UNITY_EDITOR
        if (fixImporter) {
            var importer = UnityEditor.AssetImporter.GetAtPath(assetRelPath) as UnityEditor.TextureImporter;
            if (importer == null) { UnityEditor.AssetDatabase.ImportAsset(assetRelPath); importer = UnityEditor.AssetImporter.GetAtPath(assetRelPath) as UnityEditor.TextureImporter; }
            if (importer != null) {
    ...
    // === Added by Assistant: Normal atlas export helpers ===
    public Color[] CaptureFrameNormals()
    {
        var t = StartSample();
        return CreateNormal(t);
    }

    public string ExportAtlasNormals(string roleName, string name, System.Collections.Generic.List<Color[]> frames, int columns, bool fixImporter = true, bool startFromTop = false)
    {
        if (frames == null || frames.Count == 0)
        {
            Debug.LogWarning("ExportAtlasNormals: no frames to export.");
            return null;
        }
        int frameW = SampleWidth;
        int frameH = SampleHeight;
        ...
        Texture2D atlas = new Texture2D(columns * frameW, rows * frameH, TextureFormat.ARGB32, false);
        atlas.filterMode = FilterMode.Point;
        BlitFramesToAtlas(atlas, frames, columns, startFromTop);
        atlas.Apply();
        string dir = Application.dataPath + "/Export/" + roleName;
        System.IO.Directory.CreateDirectory(dir);
        byte[] bytes = atlas.EncodeToPNG();
        string fileName = name + "_atlas_n.png";
        string path = System.IO.Path.Combine(dir, fileName);
        System.IO.File.WriteAllBytes(path, bytes);
        string assetRelPath = "Assets/Export/" + roleName + "/" + fileName;
        Debug.Log("Normal atlas saved to: " + path);
        #if UNITY_EDITOR
        if (fixImporter) {
            var importer = UnityEditor.AssetImporter.GetAtPath(assetRelPath) as UnityEditor.TextureImporter;
            if (importer == null) { UnityEditor.AssetDatabase.ImportAsset(assetRelPath); importer = UnityEditor.AssetImporter.GetAtPath(assetRelPath) as UnityEditor.TextureImporter; }
            if (importer != null) {
                ...
                importer.filterMode = FilterMode.Point;
                importer.textureCompression = UnityEditor.TextureImporterCompression.Uncompressed;
                importer.sRGBTexture = false; // normals should be linear
    ...
    // === Added by Assistant: shared blit for color/normal atlases ===
    void BlitFramesToAtlas(Texture2D atlas, System.Collections.Generic.List<Color[]> frames, int columns, bool startFromTop)
    {
        int frameW = SampleWidth;
        int frameH = SampleHeight;
        if (columns <= 0) columns = frames.Count;
        ...
        for (int i = 0; i < frames.Count; i++)
        {
            ...
            int row = i / columns;
            int x = col * frameW;
            int y = startFromTop ? ((rows - 1 - row) * frameH) : (row * frameH);
            var colors = frames[i];
            if (colors == null || colors.Length != frameW * frameH) continue;
            atlas.SetPixels(x, y, frameW, frameH, colors);
        }
